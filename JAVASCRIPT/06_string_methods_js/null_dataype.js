// ############## Null datatype ####################3
   
// what is the null in javascript ?
// Null is an object in js and represents primitive datatype 
// .a nullvalue in javascript is used for referring absence of any object 
// value and if any function or any variable returns null
// ,then we can refer that the object could not be cereated


let myvar = null;
console.log(myvar); //null
console.log(typeof myvar,myvar); // Object Null

let mynull = null;
console.log(typeof mynull); //   object
// bug . error

/*
why is null type is an object ?
the reason is behind that is that null ,
in contrast with undefined , was (and still is) often
used where objects appear .In other words , null if often
used to signify an empty reference to an object

When Briden Eich created javascript ,
he followed the same paradigm , and it made sense (arguably)
"object".
*/

// ######################3 BigInt ######################33333

let mynum = 123;

console.log(mynum);
console.log(Number.MAX_VALUE_SAFE_INTEGER);

// declaring BigInt
let num = BigInt(315);
let num1 = 123n;
console.log(num);
console.log(num + num1);
let num3 = 1245;
console.log(num + num3); /// invalid 
// error cannot mix bigint with other types.

/*
BigInt is a built-in object in JavaScript that provides 
a way to represent whole numbers larger than 2^53-1.
 The largest number that JavaScript can reliably 
 represent with the Number primitive is 2^53-1,
 which is represented by the MAX_SAFE_INTEGER constant.

*/


